...
const FeatureItem:React.FC<FeatureItemTypes> = ({name, ...props}) => {
    ...
    const itemImage = name === "UsePoint" ? StaticImages.earn_point
        : name === "RegisterMembership" ? StaticImages.register_card
            : name === "LinkMembership" ? StaticImages.link_card
                : name === "TransferPoint" ? StaticImages.transfer_point
                    : StaticImages.fallback_img

    const itemLabel = name === "UsePoint" ? "Tích, dùng điểm"
        : name === "RegisterMembership" ? "Đăng ký thẻ"
            : name === "LinkMembership" ? "Liên kết thẻ"
                : name === "TransferPoint" ? "Đổi điểm"
                    : "..."

    return (
        <Pressable onPress={_onPressHandler} _pressed={{opacity: 0.5}} flexDirection="column" alignItems="center" {...props}>
            <ImageStatic uri={itemImage} width={16} height={16} borderRadius={100}/>
            <Text mt={2} textTransform="uppercase" textAlign="center">{itemLabel}</Text>
        </Pressable>
    )
}
