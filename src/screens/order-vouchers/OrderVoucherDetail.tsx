...
import {Text, Box, ScrollView, VStack, Center, HStack} from "native-base";
import {Formatter, OrderVoucherType, PromotionType, ScreenSize, SonkimApiService} from "../../share";
import {useNavigation, useRoute} from "@react-navigation/native";
import {Alert} from "react-native";
import {Dialog, FullScreenLoader, HTMLContent, Image, MyButton, PriceDisplay, QrCode, Typo} from "../../components";
        ...
        <Box flex={1} bgColor="white" >
            <ScrollView>
                <Box m={4}>
                    <VStack>
                        <Center>
                            <Typo type="title" textAlign="center" mb={5} color="primary.500" textTransform="uppercase">{promotion.title}</Typo>
                            ...
                            <Typo type="title" textAlign="center" mt={5} textTransform="uppercase" letterSpacing={4}>{code}</Typo>
                        </Center>

                        <Box bgColor="rgba(240, 240, 240, 0.5)" mt={5} p={4} rounded="lg">
                            <HStack alignItems="center">
                                <Typo flex={1} type="body14" color="gray.600">Hạn sử dụng:</Typo>
                                <Typo flex={2} type="subtitle14">{Formatter.FormatDateFromDate(new Date(expired_at))}</Typo>
                            </HStack>

                            <HStack alignItems="center" mt={2}>
                                <Typo flex={1} type="body14" color="gray.600">Chi nhánh:</Typo>
                                <Typo flex={2} type="subtitle14">
                                    Áp dụng cho <Text underline color="red.700">1 chi nhánh</Text>
                                </Typo>
                            </HStack>

                            <HStack alignItems="center" mt={2}>
                                <Typo flex={1} type="body14" color="gray.600">Thương hiệu:</Typo>
                                <Typo flex={2} type="subtitle14">{""}</Typo>
                            </HStack>
